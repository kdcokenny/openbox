...
async def run_example(file: Path, local: bool = False):
    process = await asyncio.create_subprocess_exec(
        ...
        file.absolute(),
        env={
            "CODEBOX_API_KEY": "local"
            if local
            else os.environ["CODEBOX_API_KEY"]
        },
    )
    await process.wait()
    ...
    if process.returncode != 0:
        raise Exception(
            f"Example {file} failed with return code {process.returncode}"
        )
...
async def run_examples():
    if os.environ.get("CODEBOX_API_KEY") is None:
        return print(
            "Skipping remote examples because CODEBOX_API_KEY is not set"
        )
